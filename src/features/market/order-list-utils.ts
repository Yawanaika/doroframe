...
export function whisper(order: ItemOrder, nameZh: string, nameEn: string): string {
    ...
    const rank = order.rank !== undefined ? `(${t("market.column.rank")}: ${order.rank})` : "";
    return zh
        ? `/w ${who} 你好! 我想以 ${order.platinum} 白金 ${def.actionZh}: "${item} ${rank}"。(warframe.market)`
        : `/w ${who} Hi! I want to ${def.actionEn}: "${item} ${rank}" for ${order.platinum} platinum. (warframe.market)`;
}
